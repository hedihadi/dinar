...
class SelectServer_Server extends StatefulWidget {
  SelectServer_Server({Key? key, required this.server}) : super(key: key);
  Server server;
  @override
  State<SelectServer_Server> createState() => _MyServerState();
}

class _MyServerState extends State<SelectServer_Server> {
  late Server server;
  @override
  void dispose() {
    super.dispose();
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
                  ...
                  child: Align(
                    ...
                    child: IconButton(
                        onPressed: () async {
                          ...
                          setState(() {
                            widget.server.favorited = !widget.server.favorited;
                          });
                        },
                        icon: widget.server.favorited == false ? Icon(Icons.bookmark_add) : Icon(Icons.bookmark_remove),
                        color: widget.server.favorited == false ? Colors.green[400] : Colors.red[300]),
                  ),
                ),
              ],
            ),
            ...
            ElevatedButton(
              onPressed: () {
                context.read<AddNewNotificationProvider>().set_server(server);
              },
              child: Text("Select"),
              style: ButtonStyle(backgroundColor: MaterialStateProperty.all(ColorManager().background)),
            )
          ],
        ),
      ),
